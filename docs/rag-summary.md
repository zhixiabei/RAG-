...
召回侧文档命中率 98%、Chunk 命中率 94%，但 MRR 只有 70%，说明“能召回”明显好于“把正确证据排在前面”；答案侧正确性和完整性均为 84%，忠实性为 88%，说明回答基本受证据约束，但仍有一部分题目存在排序、覆盖或答案组织问题。通过率 79% 低于答案评分 85%，也说明加权平均分不能替代逐题通过率。
