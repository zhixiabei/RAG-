...
| `QDRANT_DOCUMENT_COLLECTION` | 文件路由 collection；每个文件包含身份、摘要、主题和章节路由点 | 默认在 chunk collection 后追加 `_documents` |
...
| `RAG_QUERY_PLANNING_ENABLED` | 让决策模型在一次调用内按需生成多轮指代改写或复合问题拆解；简单查询不额外调用模型 | `true` |
